...
- **Fuzzing:** brute-force mutated input for the critical parsers/mutators —
  ...
  **`proptest` in-crate** first (no nightly, runs in `make check`): both targets
  ...
  invariant), then the path/`%`-template expanders (#438). **Six targets total,
  all clean.** `find_match` + `diff_render` stay on unit/proptest coverage —
  they're methods over a constructed model, not byte-parsers, so awkward to
