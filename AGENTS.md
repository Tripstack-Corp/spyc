...
- **`src/app/`** — The application layer. Decomposed from a former ~12k-line `mod.rs` monolith and migrated to MVU: `App` now owns three disjoint fields — `state: AppState` (Model), `runtime: Runtime` (OS handles/channels/PtyHosts), `view: ViewState` (render ephemerals/caches). `mod.rs` (~3.7k) holds those struct defs, `App::new`, the event-driven `run` loop, and small glue; everything below is a child module whose `impl App` methods read `App`'s private state via the descendant-module rule (so fields stay private — only the few cross-module entry points are `pub`). **New handler/render/command logic goes in the matching module below — or a new one — not back into `mod.rs`** (a test guards its line count; see Conventions).
  - **`mod.rs`** — `App` / `Runtime` / `ViewState` struct defs, `App::new`, the `Message` enum + `run` event loop (recv → dispatch → effects → render), layout glue, and a few not-yet-homed helpers (term-title, agent resume strippers/resolvers, history popups, worktree list).
  - **`state.rs`** — `AppState`, the **Model**: pure domain state (cursor, picks, listing, mode, filter, focus, git display state) with no OS handles. The pure/testable Update half of MVU (`apply` / `dispatch_command` / `dispatch_prompt`), plus the `COMMAND_TABLE` `:`-command registry.
  ...
  - **`route.rs`** — pure `KeyEvent` → destination routing (a `Copy` snapshot + pure fn + regression tests; tested without a TUI). The template for further pure-decision extraction.
...
- **Keep `src/app/` modularized (don't regrow the monolith)**: `app/mod.rs` was a ~12k-line monolith; the `REFACTOR_PLAN.md` decomposition + the MVU migration carved it down to ~3.7k (just the `App`/`Runtime`/`ViewState` defs, `App::new`, the `run` loop, and glue). New render/key/command/action/session logic belongs in the matching child module (or a new `src/app/<feature>.rs`), **not** appended to `mod.rs`. The pattern is a child module with `impl App { … }`: child modules can read `App`'s private fields via the descendant-module rule, so you almost never need to make a field `pub` — only the handful of methods called from `app` or sibling modules. A test (`app::guard_tests::mod_rs_stays_decomposed`) fails if `mod.rs` grows past its ceiling; if you hit it, extract a module rather than bumping the number.
- **`:command` registration goes through `COMMAND_TABLE`** (`state.rs`): every `:`-command is one `CommandSpec { name, layer, completion }` entry, where `layer` is `Pure` (handled in `AppState::dispatch_command`) or `App` (terminal-touching, handled in `App::dispatch_command`, `src/app/commands.rs`). State runs first; the table's `command_layer(name)` drives both tab-completion and the Pure→App routing, so you no longer hand-sync a separate punt list — you add a table entry plus its handler arm. A missing **Pure** arm is caught by the `command_table_*` tests; a missing **App** arm still flashes "unknown command: X" at runtime (the remaining gap — the last-mile pass moves the handler into the table so this becomes a compile error). Symbol commands (`!`, `;`, `!!`) are dispatched directly and stay out of the table. Bitten historically on `:undo` (v1.41.1) and the `:limit`/`:`-history split.
