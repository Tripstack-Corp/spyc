...
- **Agent-activity dots** — each agent tab carries a live activity dot: P0 output-timing (spicy heat-pulse `●` while output flows, quiet `·` when idle) overridden by P1 semantic self-report via the `report_status` MCP tool (`Working`/`Blocked`/`Done`). `:why-status` explains the active tab (`:activity dump` → every pane's why-status in a yankable/saveable pager — the diagnostic for "why isn't this dot red?"); a `^z`-suspended tab shows 💤 instead (see Pane I/O). For *auto*-report, spyc injects `SPYC_MCP_SOCK` + a stable `SPYC_PANE_ID` into the agent pane and ships a one-shot `spyc --report-status <state>` reporter (`mcp::report_status_to_socket`). **Consent-gated** (`mcp::hooks` + `state::hook_consent`): a `[Y/n]` popup on the first claude launch per repo (saved; y/n required — Esc/other keys keep it up, a saved `n` is undoable) before any `.claude/settings.json` hook write; `:hooks on|on!|off` changes it after the fact (`on!` = `force_restart_status_hooks`, restart+resume the pane when a live-reload doesn't take). Charter: `docs/AGENT_AWARENESS_PLAN.md`.
