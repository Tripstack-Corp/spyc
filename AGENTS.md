...
- **`src/mcp/`** — MCP server (`mod` facade + `run`/socket paths + the `SERVER_*` consts, `server` socket transport, `protocol` JSON-RPC handlers, `config` `.mcp.json`/codex management + enterprise policy + instance takeover, `readers` context-file readers): PID-scoped Unix socket listener, stdio proxy for Claude Code. The `initialize` response carries an `instructions` field (`SERVER_INSTRUCTIONS`) folded into the launched agent's system prompt — the ephemeral, no-files-written way to steer it toward spyc's tools (prefer `search_content`/`navigate_to`/`create_worktree` over `Bash rg`/`cd`/`git worktree`) instead of ignoring them. Keep it short; clients truncate it.
