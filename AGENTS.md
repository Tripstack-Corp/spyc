...
- **`src/git/`** — Git integration facade: the single boundary owning every git operation (`status.rs`, `worktree.rs`, `diff.rs` = diff/show/blame, `discovery.rs` = repo-root/gitdir/branch via gix). Pure infra (paths in, owned data/bytes out — no `App`, no ratatui). Being migrated from `git`-binary shell-outs to the `gix` (gitoxide) crate domain-by-domain behind this seam (so call sites never change again); **discovery, status, and worktree (list/create/remove) already run on gix**. For diff/show/blame the gix **data model** now exists — `model.rs` (pure owned `DiffModel`/`BlameModel`/`CommitMeta`, the contract the renderer consumes), `diff_model/` (gix→`DiffModel` for `gd`/`gD`/`show`, built on `gix-diff` + imara-diff hunk reassembly), `blame.rs` (gix `blame_file` → `BlameModel`) — but the **live** diff/show/blame path in `diff.rs` still shells out; the model is `#[allow(dead_code)]` until PR 8 wires the in-house renderer (then PR 9 deletes the subprocess bodies). `status.rs` keeps an `SPYC_GIT_BACKEND=subprocess` escape hatch for the gix-status soak. App-layer git glue — worker-result application, pager mounting, error flashing — stays in `src/app/git_state.rs`. **Hot-path rule:** the 1 Hz git mtime poll reads the cached `current_gitdir` (no gix open); gix opens only at chdir-into-a-new-repo + HEAD change.
