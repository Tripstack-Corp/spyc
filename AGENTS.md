...
- **`src/git/`** — Git integration facade: the single boundary owning every git operation (`status.rs`, `worktree.rs`, `diff.rs` = diff/show/blame). Pure infra (paths in, owned data/bytes out — no `App`, no ratatui). Currently shells out to the `git` binary; being migrated to the `gix` (gitoxide) crate domain-by-domain behind this seam (so call sites never change again). App-layer git glue — worker-result application, pager mounting, error flashing — stays in `src/app/git_state.rs`.
