...
- **`src/git/`** — Git integration facade: the single boundary owning every git operation (`status.rs`, `worktree.rs`, `diff.rs` = diff/show/blame, `discovery.rs` = repo-root/gitdir/branch via gix). Pure infra (paths in, owned data/bytes out — no `App`, no ratatui). Being migrated from `git`-binary shell-outs to the `gix` (gitoxide) crate domain-by-domain behind this seam (so call sites never change again); `discovery.rs` already runs on gix. App-layer git glue — worker-result application, pager mounting, error flashing — stays in `src/app/git_state.rs`. **Hot-path rule:** the 1 Hz git mtime poll reads the cached `current_gitdir` (no gix open); gix opens only at chdir-into-a-new-repo + HEAD change.
