...
- **`src/context.rs`** — Context snapshot (cwd, cursor, picks, filter, git branch, project_home, search_root, session_name) written to disk for MCP consumers. `search_root` is the focused column's worktree root, so MCP search follows the worktree (grep `F` / find use `AppState::tool_root` directly).
