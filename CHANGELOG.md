...
### Features
- **skill**: Installable agent skill (claude + codex) via --install-skill, with a startup update offer (#187)
- **agent**: Agy MCP config, scrape-fallback blocked detection, worktree-aware session restore — and drop deprecated gemini support (#194)
- **agent**: Pin agy's conversation from its hook payload, not spawn proximity (#202)
- **mouse**: Terminal mouse-reporting plumbing, inert until capture is enabled (#212)
- **mouse**: Wheel scrolls whatever is under the pointer (#213)
- **mouse**: Left-click focuses, middle-click pastes, right-click opens the leader (#214)
- **mouse**: Forward drags, so a child's own text selection works (#224)
- **mouse**: Drag to select text in the pager, release to copy (#227)
- **mouse**: Drag to select file-list rows, click to copy the status line (#233)
- **mouse**: Drag to select text in a pane whose child ignores the mouse (#234)
- **about**: An about page on `Space a` / `:about` (#254)
- **clipboard**: User command override + bounded helper reap (#261)
- **mouse**: Invert_scroll config toggle (#225) (#260)
- **mouse**: Click a pane tab to switch to it (#279)
- **mouse**: Select and copy the prompt row and the activity HUD (#281)
- **agent**: Report agy's blocked dot from its ask_question hook (#287)
- **list**: Lead the L long listing with NAME, not trail it (#293)
- **search**: `^` and `$` anchor `/` and `=` queries (#296)
- **pager**: Preview image files full-screen, on one shared image path (#300)
- **pane**: `^a g` gallery of the images the agent actually received (#302)
- **archive**: Archive-browsing core — index, journal, listing, safety (#301)
- **pane**: Keep a copy of the images you paste into an agent (#304)
- **archive**: Enter a zip or tarball as if it were a directory (#305)
- **archive**: Take things out of an archive — yank, copy out, pipe (#307)
- **archive**: Write an archive back — delete a member, then repack (#310)
- **archive**: Put, rename and edit a member — the rest of write-back (#311)
- **archive**: Let an agent read a member of a mounted archive (#314)
- **archive**: Come back to a place inside an archive, mounting it on the way (#317)
- **archive**: Enter an archive inside an archive (#319)
- **archive**: Mark a changed member in the listing, not just in the suffix (#328)
- **archive**: A yanked member is remembered by its path in the archive (#333)
- **archive**: Write or discard an archive without being inside it (#334)
- **git**: Mark the main worktree in the W l picker, and tilde-collapse its paths (#340)
- **vsplit**: The split opens full-height, and ^s owns the whole family (#352)
- **diff**: Per-character intraline highlight by default, `[diff] intraline` to choose (#364)
- **pane**: An inline agent's transcript is reachable, and T swaps sources (#391)

### Bug Fixes
- **listing**: Surface why a directory read failed, not just which one (#190)
- **app**: Every flashed error keeps its cause, and a guard to keep it that way (#191)
- **git**: Resolve the branch ref in the marker poll so commits trigger a re-walk (#193)
- **agent**: Debounce the agy approval-prompt scrape so a half-drawn screen can't flip a dot (#195)
- **agent**: Agy status-dot, session-restore, and MCP config-sweep defects (#201)
- **agent**: Validate agy's live session pin against its transcript, not history.jsonl (#203)
- **ui**: Keep the side-by-side diff separator on one column for tab-indented files (#205)
- **diff**: Measure split-column wrap in grapheme clusters, not chars (#215)
- **agent**: Don't let a live report cancel the scrape scan agy depends on (#216)
- **mcp**: Refuse to overwrite an agent config we can't parse (#217)
- **mouse**: Make the terminal's mouse state authoritative on every path (#218)
- **mouse**: Hit-test the real frame, and encode buttons as buttons (#219)
- **mouse**: Give the snapshot the three fields it was missing (#221)
- **version**: Name the exact build everywhere a version is shown (#222)
- **mouse**: Scroll the agents that ignore mouse reports, per verified keybinding (#226)
- **mouse**: An open pager outranks the region painted beneath it (#228)
- **mouse**: The pointer's surface owns the event, the wheel stops at the ends, and codex scrolls (#229)
- **mcp**: Validate the root override against the session's roots (review F1) (#237)
- **state**: Route every persistent write through write_atomic (review F3) (#238)
- **mcp**: Unify on one state-root resolver (review F6) (#239)
- **ci**: Build the fuzz targets for gnu, not musl (review F5 follow-up) (#242)
- **test**: Stop the test suite writing to the real repo under a git hook (C9) (#248)
- **test**: Give the source-scan guards a splitter that doesn't fail open (C2) (#249)
- **pager**: Put [EOF] on the last line of a markdown file, not the row below (#251)
- **git**: Fold the shared config's mtime into the status cache key (C8) (#255)
- **codex**: Pin a resumed session by liveness, not start time (C3, #230) (#250)
- **agent**: Clear a blocked dot when the prompt is declined, not just answered (#257)
- **agent**: Let `^c` clear a blocked dot too, alongside Enter and Esc (#258)
- **pane**: Honor the child's DECCKM when encoding cursor keys (#259)
- **hooks**: Strip GIT_REDIRECT_ENV before make check (companion to C9, #248) (#262)
- **diff**: Wash the shorter side's wrap padding in a split diff (#263)
- **mouse**: Only a sustained scroll UP opens codex's transcript (#264)
- **codex**: Rank a fresh pane's rollout by identity, not activity (#230) (#268)
- **pager**: Paint the full row for a blank line in visual-line mode (#120) (#271)
- **pager**: Flash pager-action messages in the pager, not the occluded status bar (#166) (#270)
- **pane**: Confirm ^a R on a live child and restart in place (#151) (#269)
- **pane**: Don't double-source rc files when the pane command is a shell (#275)
- **term**: Restore the terminal on SIGTERM / SIGHUP (#277)
- **git**: Worktree-removal refusals name their cause (#278)
- **cli**: --version prints the git SHA the docs promise (#283)
- **agent**: Restore agy's blocked dot by dropping a needle agy never prints (#285)
- **pager**: Restore the [EOF] end-of-file signal at the exact-fit boundary (#286)
- **list**: A watcher refresh no longer reverts the sort order to name (#289)
- **pager**: Size the fit-to-content box from visual rows, not line count (#290)
- **mouse**: Send codex's transcript close key once, not once per wheel tick (#292)
- **prompt**: Keep the cursor at the buffer end after cw/ce (#267) (#294)
- **prompt**: Draw a real underscore where the vi prompt's cursor sits (#295)
- **git**: One freshness key for the marker poll (stops a 1 Hz forever-walk) (#303)
- **pane**: Drop the exit tag on restart + `^z` suspends any non-shell tab (#306)
- **pane**: Make the image gallery a popup, not a view of the frame (#308)
- **test**: Stop the anchor proptest flaking on names ending in a decoration (#309)
- **pane**: The image overlay was undismissable — put it on the modal axis (#313)
- **archive**: A mount path is neither a member nor a real file (#312)
- **image**: Write viewer temp files to a private dir, not a guessable path (#316)
- **transcript**: Bound the image index's line reader (#318)
- **agent**: Keep a live pane's status hooks when a sibling spyc exits (#315)
- **archive**: An edited member is a pending change, and the badge says so (#320)
- **archive**: Notice an edited member however it was edited (#321)
- **status**: The status bar describes the focused column, not always column a (#322)
- **archive**: Mark a dirty archive in the directory it lives in (#329)
- **archive**: A streamed tar's directory entries survive the write-back (#330)
- **archive**: Refill staging from the archive when a member goes missing (#335)
- **archive**: Discard throws away the staged copies, not just the journal (#336)
- **archive**: A put member can be read, edited and deleted (#337)
- **archive**: A write-back empties its staging tree in the same op, not beside it (#338)
- **archive**: An in-flight message stops claiming the work is still running (#339)
- **mcp**: The worktree tools anchor on the project, not on an open archive (#341)
- **mouse**: A chrome selection echoes what was taken, then says it copied it (#342)
- **archive**: Refuse a directory copy-in instead of losing what is inside it (#343)
- **archive**: Decide containment against the filesystem, not the name (#347)
- **archive**: A declared size is a claim, so stop spending on it (#348)
- **mouse**: The capture guard was blind to the file that sets the terminal up (#349)
- **clipboard**: Every yank goes through the one delivery seam (#350)
- **app**: The flash guard reads the whole call, so a wrapped one can't hide (#353)
- **app**: The module-index guard can see subdirectories now (#357)
- **diff**: Highlight every changed word in a line, not one blob between them (#351)
- **ui**: Measure and cut text by grapheme cluster, not by char (#354)
- **clipboard**: A middle-click paste reads the clipboard off the loop, bounded (#362)
- **session**: A live codex tab saves the rollout it was pinned to, not nothing (#363)
- **graveyard**: The startup cascade walked the directory once per entry (#367)
- **graveyard**: Run the cap cascade off the startup thread (#370)
- **mouse**: Copying part of a flash keeps the message, instead of eating it (#372)
- **mouse**: A chrome row's hit area has a right edge, so column b keeps its clicks (#375)
- **archive**: A streamed member's bytes go where its own entry says (#376)
- **archive**: A member can't squat on the staging escape namespace (#377)
- **mouse**: Invert_scroll reaches the agent pane too, not just spyc's own surfaces (#378)
- **pane**: The scrollback snapshot settles on the loop, not in a 30 ms sleep (#379)
- **clipboard**: The reap budget bounds the write, not just the wait (#380)
- **clipboard**: A yank stops waiting out the helper on the event loop (#381)
- **git**: The env-hygiene guard requires the one spawn site, not 1 of 9 variables (#383)
- **mouse**: A wheel tick over an inline agent scrolls spyc's own history (#387)
- **archive**: Staging carries spyc's permissions, the index carries the archive's (#392)
- **archive**: A put member's row describes the file, not a placeholder (#393)
- **session**: A split whose column was inside an archive comes back (#394)
- **archive**: The entry cap bounds the index, not just the walk (#396)
- **skill**: A torn manifest write clobbers the edits it promised not to (#401)
- **clipboard**: A yank's done-wake panics the loop instead of coalescing (#402)
- **keymap**: A chord that binds an action now has to advertise it (#408)
- **archive**: The write-back's verification honours the budget it ignored (#410)
- **archive**: HIGH-1's third site was never fixed, and nothing could tell (#412)
- **pane**: Scrolling past the end of a scrollback returns you to the live pane (#413)
- **mcp**: The read cap bounds the bytes, not the archive's claim about them (#414)
- **archive**: A link target is resolved through links, not folded on paper (#417)

### Performance
- **health**: The startup scan stops opening every metadata file (#371)

### Refactors
- **mouse**: Split the pure routing layer into mouse/route.rs (F4 Part B, 1/n) (#244)
- **mouse**: Split the drag-selection clusters into mouse/selection.rs (F4 Part B, 2/n) (#245)
- **mouse**: Split scroll and forwarding out, completing the decomposition (F4 Part B, 3/n) (#246)
- **mouse**: Move the test clusters to their post-split subjects (C4) (#252)
- Pure key decisions for the harpoon menu + find picker (#178) (#297)
- **pane**: Read a macOS pane's cwd in-process instead of spawning lsof (#356)
- **paths**: Expand ~ and $VAR via shellexpand instead of by hand (#358)
- **shell**: Quote shell words via shlex instead of by hand (#360)
- **config**: One config-root resolver, and drop the dead hostname shell-out (#361)
- **app**: Every off-thread wake is one Message variant, not seventeen (#405)
- **app**: Mod.rs had five lines of headroom; extract instead of bump (#406)
- **app**: Mod.rs holds the module root, not the two big field types (#407)

### Documentation
- **readme**: How to install and run the CURRENT stream (#188)
- Recommend single-branch clone to avoid large downloads (#192)
- **agent**: Correct the transcript-miss behavior and agy's transcript path (#204)
- Add native scroll implementation plan (#200)
- Rescope the native-scroll plan around forwarding to the child (#207)
- Mouse capture ships default-on, flipped once the feature is ready (#208)
- Add the three mouse button gestures to the native-scroll plan (#209)
- Right-click opens the leader menu; row-select rejected (#210)
- Rebindable mouse buttons (incl. Lua), and restore three lost edits (#211)
- Plan mouse text selection (the other half of capture) (#223)
- **mcp**: Record the root-override scoping decision (review F1) (#235)
- **security**: Describe the shipped distribution and signing chain (review F2) (#236)
- **roadmap**: The 800-LoC rule is a convention, not a finished foundation (review F4) (#241)
- **agents**: Trim the feature section toward the pointer doc it claims to be (review F7) (#243)
- **agents**: Warn against `git reset FETCH_HEAD` (#256)
- **release**: Draft the human-facing 2.1 release notes (#266)
- **harness**: Add the per-agent harness guide (#232) (#272)
- Fill in the missing keybindings + fix archive relative links (#276)
- **release**: Refresh the 2.1 release notes for what actually shipped (#282)
- **features**: The git gutter's glyphs are spyc's, not porcelain's (#284)
- **release**: Add today's fixes to the 2.1 release notes (#299)
- **archive**: Charter for navigating into zip/tarballs (#149) (#298)
- **readme**: A four-loop tour of the commander, not just the bridge (#323)
- **agents**: The merge-wait loop has to require a non-empty check list (#344)
- **agents**: Create_worktree branches off LOCAL main, so pull first (#345)
- **readme**: A real session, in one frame (#355)
- **agents**: Leave no target/ behind — and fuzz/ needs its own clean (#366)
- **agents**: With a shared build dir, never cargo clean — it deletes everyone's (#373)
- **release**: Bring the 2.1 notes up to HEAD — archives, images, the review (#374)
- **features**: Agy's dot does report blocked — FEATURES said the opposite (#382)
- **security**: SECURITY.md claims two things HEAD doesn't do (#384)
- Close the 2.1 doc-contract cluster — five findings, one pass (#386)
- **plan**: The mouse-binding tokens were dropped — say so where they were specified (#389)
- **mouse**: Four comments describing a mode spyc does emit, two describing the wrong function (#400)
- Close the 2.1 low doc-contract cluster — eight findings, one pass (#403)
- **review**: Add lf to the competitive review + retire the mouse non-goal (#390)
- **readme**: Slim the README to a storefront, relocate the reference detail (#404)
- The last three informational findings — say the quiet parts (#411)
- **agents**: A test is evidence only once you've watched it fail (#415)
- **review**: The pre-2.1 review is closed and archived — the eighth pass (#421)
- **archive**: File the Aug-2026 engagement records, minus the two still live (#422)
- **release**: Finalize the 2.1 notes + track the two drafts that still carry open work (#423)

### Build & Tooling
- **deps**: Bump toml from 1.1.3+spec-1.1.0 to 1.1.4+spec-1.1.0 in the cargo-minor-patch group (#197)
- **make**: Check-ci + document the merge race (C5, C6) (#253)
- **deps**: Bump the cargo-minor-patch group with 5 updates (#332)
- Ask cargo where target/ is, so a shared build dir works (#369)
- **deps**: Bump two-face from 0.5.1 to 0.5.2+bat-0.26.1 in the cargo-minor-patch group (#420)

### Tests
- **git**: Report the lock directory state when a worktree index write fails (#220)
- **input**: Pin paste-into-pager-search routing and vi `cw` in prompts (#265)
- **clipboard**: Retry a stub exec refused with ETXTBSY (#325)
- **archive**: Assert the pending-change badge is painted, not just recorded (#324)
- **fuzz**: Fuzz the archive container parsers, not just member names (#346)
- **render**: The purity guard can see the draw pass mutate (#359)
- **archive**: Pin the three write-back safety properties a mutation could gut (#365)
- **mouse**: Cover the dispatch entry, the press/release pairing, and the wheel-to-keys caller (#368)
- **git**: A stale pre-commit hook fails the gate instead of hiding (#385)
- **archive**: A corrupt DOS date is tested against spyc, not against the zip crate (#388)
- **guards**: The per-column allowlist stopped guarding the file it was written for (#395)
- **archive**: Pin why an escaping link doesn't make the mount read-only (#397)
- Three assertions that couldn't tell the pass from the failure (#398)
- **archive**: The write-time free-space precheck was never once exercised (#399)
- **guards**: The module index can be satisfied by a longer sibling (#409)
- **guards**: Tighten four guards + narrow relocated predicates + document ^a C (#418)

### Miscellaneous
- **apt**: Prune superseded prerelease debs on every publish (#189)
- **ci**: Bump taiki-e/install-action from 2 to 2.85.5 in the github-actions group (#196)
- Remove accidental test_gix.rs (#198)
- **ci**: Bump taiki-e/install-action from 2.85.5 to 2.85.10 in the github-actions group (#331)
